...
func GetFeatureVectorPoint( image_bytes *[]byte ) ( features []float64 ) {
	img , err := gocv.IMDecode( *image_bytes , gocv.IMReadColor )
	if err != nil { return }
	if img.Empty() { img.Close(); return }
	defer img.Close()
...
func CalculateDistancePoint( reference_image_features *[]float64 , test_image_features *[]float64 ) ( result float64 ) {
	result = chi2Distance( *reference_image_features , *test_image_features )
	return
}